	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//processing R2  <?a 21508 14193>
				keyString=21508.toString()+14193.toString();				
				if(poRoot(here.id).hasChild(keyString)){
					R0_Node=poRoot(here.id).getChild(keyString);
					R0_tuple=R0_Node.getData();
				}
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//print R1
				var opath:String="/data/RDF_Processing/data_1b/index2/lubm_q13/"+pn.toString()+".R1";
				...
				//print R2
				opath="/data/RDF_Processing/data_1b/index2/lubm_q13/"+pn.toString()+".R2";
