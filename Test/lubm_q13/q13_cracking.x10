	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				var ns:String;
				val s2="/data/RDF_Processing/data_1b/index2/lubm_q13/"+pn.toString()+".R1.gz";
				...
				//build for R2 <?a 21508 14193>
				val s3="/data/RDF_Processing/data_1b/index2/lubm_q13/"+pn.toString()+".R2.gz";
				lstring=gzRead(s3);
				len=lstring.length();
				start=0;
				end=0;
				postring= 17528.toString()+20871.toString();
				tmpoNode=new poNode(postring);
				while(start<len) {
					end=lstring.indexOf('\n',start);
					line=lstring.substring(start,end);
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//build R2 hash set  <?a 21508 14193>
				var R0_Node:poNode;
				var R0_tuple:ArrayList[Long];
				var postring:String=21508.toString()+14193.toString();				
				...
				//check R1 in R2_hashset <?a 20758 8799>
				postring=20758.toString()+8799.toString();
				if(poRoot(here.id).hasChild(postring)){
					R0_Node=poRoot(here.id).getChild(postring);
					R0_tuple=R0_Node.getData();
