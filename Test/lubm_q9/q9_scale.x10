	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				for(loop in 0..(FILE-1)){
					e3=pn+N*loop;
					
					for(e in 1..6){
						val ee=e;
						var ns:String;
						val s1="/data/RDF_Processing/data_1b/index2/lubm_q91/"+e3.toString()+".R"+ee.toString()+".gz";
						...
						case 2: 
							p_index=12940L;
							if(pRoot(here.id).hasChild(p_index)){
								tmpNode=pRoot(here.id).getChild(p_index);
								while(start<len) {
									end=lstring.indexOf('\n',start);
