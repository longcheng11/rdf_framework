	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//print R2
				var opath:String="/data/RDF_Processing/data_1b/index2/lubm_q91/"+pn.toString()+".R2";
				...
				//print R5
				opath="/data/RDF_Processing/data_1b/index2/lubm_q91/"+pn.toString()+".R5";
				OutFile=new File(opath);
				val pr3=OutFile.printer(true);
				for(i in 0..(N-1)){
					var s3:Int=R5_keys_receive(here.id)(i).size;
					for(var j:Int=0;j<s3;j++){
						pr3.println(R5_keys_receive(here.id)(i)(j).toString()+"\t"+R5_payload_receive(here.id)(i)(j).toString());		
					}
				}
				pr3.flush();
				pr3.close();
				...
				//print R3
				opath="/data/RDF_Processing/data_1b/index2/lubm_q91/"+pn.toString()+".R3";
				OutFile=new File(opath);
				val pr5=OutFile.printer(true);
				for(i in 0..(N-1)){
					var s5:Int=R3_keys_receive(here.id)(i).size;
					for(var j:Int=0;j<s5;j++){
						pr5.println(R3_keys_receive(here.id)(i)(j).toString()+"\t"+R3_payload_receive(here.id)(i)(j).toString());		
					}
				}
				pr5.flush();
				pr5.close();
				...
				//print R6
				opath="/data/RDF_Processing/data_1b/index2/lubm_q91/"+pn.toString()+".R6";
				OutFile=new File(opath);
				val pr1=OutFile.printer(true);
				for(i in 0..(N-1)){
					var s1:Int=R6_keys_receive(here.id)(i).size;
					for(var j:Int=0;j<s1;j++){
						pr1.println(R6_keys_receive(here.id)(i)(j).toString());		
					}
				}
				pr1.flush();
				pr1.close();
				...
				//print R4
				opath="/data/RDF_Processing/data_1b/index2/lubm_q91/"+pn.toString()+".R4";
				OutFile=new File(opath);
				...
				//print R1
				opath="/data/RDF_Processing/data_1b/index2/lubm_q91/"+pn.toString()+".R1";
				OutFile=new File(opath);
				val pr6=OutFile.printer(true);
				for(i in 0..(N-1)){
					var s6:Int=R1_keys_receive(here.id)(i).size;
					for(var j:Int=0;j<s6;j++){
						pr6.println(R1_keys_receive(here.id)(i)(j).toString());		
					}
				}
				pr6.flush();
				pr6.close();							
