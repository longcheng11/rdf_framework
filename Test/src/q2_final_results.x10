	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				var T2_hash_table:HashMap[Long,HashSet[Long]]=new HashMap[Long,HashSet[Long]]();
				for(i in 0..(N-1)){
					var s7:Int=T2_keys_receive(here.id)(i).size;
					for(var j:Int=0;j<s7;j++){
						key=T2_keys_receive(here.id)(i)(j);
						value=T2_payload_receive(here.id)(i)(j);
						if(r6_hash_set.contains(key)){
							if(T2_hash_table.containsKey(key)){
								T2_hash_table.get(key).value.add(value);
							}
							else{
								T2_hash_table.put(key, new HashSet[Long]());
								T2_hash_table.get(key).value.add(value);
							}
						}
					}
				...
				for(i in 0..(N-1)){
					...
					for(var j:Int=0;j<s8;j++){
						...
						valueSet=T2_hash_table.getOrElse(key,null);
				...
				var opath:String="/data/RDF_Processing/filterBuild/filters/lubm_q2/17528_4069_"+pn.toString()+".f";
