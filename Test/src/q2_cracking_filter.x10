...
public class q2_cracking_filter {
	...
	public static def main(args: Array[String]) {
		...
		Console.OUT.println("///////////////// start read filter information////////////////");
		var read_start_filter:Long=System.currentTimeMillis();
		...
		finish for (p in Place.places()){
			at (p) async {
				...
				var ns:String;
				val s1="/data/RDF_Processing/filterBuild/filters/lubm_q2/17528_4069_"+pn.toString()+".f.gz";
				var lstring:String=gzRead(s1);
				var len:Int=lstring.length();
				var start:Int=0;
				var end:Int=0;
				var line:String;
				var F_key_collector:ArrayList[Long]=new ArrayList[Long](N);
				while(start<len) {
					end=lstring.indexOf('\n',start);
					line=lstring.substring(start,end);
					start=end+1;
					F_key_collector.add(Long.parse(line));
				}
				
				//push the F to remote places
				var keys_array:Array[long];
				for( k in (0..(N-1))) {
					val kk=(k+pn)%N;
					keys_array=F_key_collector.toArray();
					val pk=Place.place(kk);
					val sf=keys_array.size;	
					at(pk){
						F_keys_receive(here.id)(pn)= new RemoteArray(new Array[Long](sf));
					}
					Array.asyncCopy( keys_array, at (pk) F_keys_receive(here.id)(pn));
				} //end pushing				
			}
		}
		var read_end_filter:Long=System.currentTimeMillis();
		Console.OUT.println("read filter time is "+(read_end_filter-read_start_filter)+" ms");
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//build local filter
				for(i in 0..(N-1)){
					var s1:Int=F_keys_receive(here.id)(i).size;
					for(var j:Int=0;j<s1;j++){
						filter(here.id).add(F_keys_receive(here.id)(i)(j));
					}
				}
				F_keys_receive(here.id)=null;
		...
		finish for( p in Place.places()){
			at (p) async {			
				...
				R_tuple=R_Node.getData();
				for(tuple in R_tuple){
					if(filter(here.id).contains(tuple(1))&& r4_hash_set.contains(tuple(0))){
