...
public class loading_2 {
	...
	public static def main(args: Array[String]) {
		...
		//triple list
		val triple_list=DistArray.make[ArrayList[Array[Long]]](d);			
		...
		finish for (p in Place.places()){
			at (p) async {
				...
				triple_list(here.id)=new ArrayList[Array[Long]]();
			}
		}		
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				while(start<len) {
					...
					start=end+1;
					triple=Parsing(line); 
					triple_list(here.id).add(triple);					
				}
			}
		}
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				for(triple in triple_list(here.id)){
					  //get the S,P,O here				
