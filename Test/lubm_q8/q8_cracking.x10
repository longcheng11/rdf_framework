	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				var ns:String;
				val s2="/data/RDF_Processing/data_1b/index2/lubm_q8/"+pn.toString()+".R1.gz";
				...
				val s3="/data/RDF_Processing/data_1b/index2/lubm_q8/"+pn.toString()+".R2.gz";
				lstring=gzRead(s3);
				len=lstring.length();
				start=0;
				end=0;
				...
				//build for R3 <?a 17528 ?b>
				val s4="/data/RDF_Processing/data_1b/index2/lubm_q8/"+pn.toString()+".R3.gz";
				lstring=gzRead(s4);
				len=lstring.length();
				start=0;
				end=0;
				...
				val s5="/data/RDF_Processing/data_1b/index2/lubm_q8/"+pn.toString()+".R4.gz";
				lstring=gzRead(s5);
				len=lstring.length();
				start=0;
				end=0;
				...
				//build for R5 <?a 5513 ?c>
				val s6="/data/RDF_Processing/data_1b/index2/lubm_q8/"+pn.toString()+".R5.gz";
				lstring=gzRead(s6);
				len=lstring.length();
				start=0;
				end=0;
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//processing R1 <?a 20758 8799> and build R1_hashtable
				...
				for(key1 in R0_tuple){
					value=T1_hash_table.getOrElse(key1,0L);
					if(value!=0L){
						R1_hash_table.put(key1,value);
					}
				}	
				...
				R_tuple=R_Node.getData();
				for(tuple in R_tuple){
					if(R1_hash_table.containsKey(tuple(0))){
						//output the results
					}
				}			
			}
