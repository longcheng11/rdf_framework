	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//build for R1 <?a 20758 8799>
				var ns:String;
				val s2="/data/RDF_Processing/data_1b/index2/lubm_q7/"+pn.toString()+".R1.gz";
				...
				//build for R2 <?a 20758 25796>
				val s3="/data/RDF_Processing/data_1b/index2/lubm_q7/"+pn.toString()+".R2.gz";
				lstring=gzRead(s3);
				len=lstring.length();
				start=0;
				end=0;
				...
				//build for R3 <?a 25782 ?b>
				val s4="/data/RDF_Processing/data_1b/index2/lubm_q7/"+pn.toString()+".R3.gz";
				lstring=gzRead(s4);
				len=lstring.length();
				start=0;
				end=0;
				...
				//build for R4 <4405 12940 ?b>
				val s5="/data/RDF_Processing/data_1b/index2/lubm_q7/"+pn.toString()+".R4.gz";
				lstring=gzRead(s5);
				len=lstring.length();
				start=0;
				end=0;
				postring= 12940.toString()+4405.toString();
				tmpoNode=new poNode(postring);
				while(start<len) {
					end=lstring.indexOf('\n',start);
					line=lstring.substring(start,end);
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//build R4 hash set <4405 12940 ?b>
				var R0_Node:poNode;
				var R0_tuple:ArrayList[Long];
				var postring:String=12940.toString()+4405.toString();				
				if(poRoot_e(here.id).hasChild(postring)){
					R0_Node=poRoot_e(here.id).getChild(postring);
				...
				//check R2 based on checking R4_hashset <?a 20758 25796> and build R2_hashset
				postring=20758.toString()+25796.toString();
