	...
	public static def main(args: Array[String]) {
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//build for R1 <?a 20758 2949>
				var ns:String;
				val s2="/data/RDF_Processing/data_1b/index2/lubm_q11/"+pn.toString()+".R1.gz";
				...
				//build for R2 <?a 4069 14193>
				val s3="/data/RDF_Processing/data_1b/index2/lubm_q11/"+pn.toString()+".R2.gz";
				lstring=gzRead(s3);
				len=lstring.length();
				start=0;
				end=0;
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				//build R2 hash set <?a 4069 14193>
				var R0_Node:poNode;
				var R0_tuple:ArrayList[Long];
				var postring:String=40696.toString()+14193.toString();				
				...
				//check R1 based on checking R2_hashset <?a 20758 2949>
				postring=20758.toString()+2949.toString();
				if(poRoot(here.id).hasChild(postring)){
					R0_Node=poRoot(here.id).getChild(postring);
					R0_tuple=R0_Node.getData();
